...
const LoopedOffthreadVideoV2: React.FC<OffthreadVideoProps> = (props) => {
  const { fps } = useVideoConfig();
  const [durationInFrames, setDurationInFrames] = useState(0);
  const [handle] = useState(() =>
    delayRender(`fetching video from: ${props.src}`),
  );
    ...
    <>
      {durationInFrames && (
        <Loop durationInFrames={durationInFrames}>
          <OffthreadVideo
            {...props}
            delayRenderTimeoutInMilliseconds={600 * 1000}
...
export default LoopedOffthreadVideoV2;
